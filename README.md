# Arduino 3D Game of Life and 3D Connect-4

This repo holds code for several 3D game of life displays and two 3D connect-4 games on a 4x4 bicolor red/blue LED cube controlled by an Arduino.
...
- Initial state:
  ...
  - One randomly selected LED flashes blue.
- Moving the joystick changes which LED is flashing to indicate your current position.
  You can only move horizontally; your vertical position adjusts automatically to occupy the lowest empty position in the column.
  Flashing purple means that the current space is already occupied by another color.
